...
namespace Jinl{
	...
	public:
		...
		explicit reverse_iterator_t(const reverse_iterator_t& it) :base_(it)
		{
			auto tmp = it;
			cur_ = --tmp;
		}
		...
		reverse_iterator_t operator-=(difference_type n);
	private:
		Iterator advanceNStep(Iterator it, difference_type n, bool right, RandomAccessIterator_tag)
		{
			if (right)
				it += n;
			else it -= n;
		...
		Iterator advanceNStep(Iterator it, difference_type n, bool right, BidirectionIterator_tag)
		{
			difference_type i;
	...
	template<class Iterator>
	reverse_iterator_t<Iterator>& reverse_iterator_t::operator +=(difference_type n)
	{
